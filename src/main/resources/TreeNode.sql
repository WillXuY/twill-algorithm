...
Node 2 is an inner node because it has parent node 1 and child node 4
and 5.

Nodes 3, 4, and 5 are leaf nodes because they have parent nodes and they
...
(SELECT id, "Leaf" as type
 FROM Tree tr
 WHERE tr.p_id is not null
 AND not EXISTS (SELECT p_id
                   FROM Tree tre
                   WHERE tre.p_id = tr.id))
;
